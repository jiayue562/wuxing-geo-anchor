...
def process_new_articles(limit=None):
    """处理新文章（默认不限制篇数，有N篇处理N篇）"""
    log("开始检查新文章...")
    ...
    # 查找所有未处理的文章（按修改时间排序，最旧的优先）
    if not WECHAT_DIR.exists():
        log(f"警告: 公众号文章目录不存在: {WECHAT_DIR}")
        return []
    ...
    # 不限制篇数：有N篇处理N篇
    new_articles = all_files
    
    if not new_articles:
        log("无新文章")
        return []
    
    log(f"发现 {len(new_articles)} 篇新文章（不限制篇数，有N篇处理N篇）")
...
def process_docs_unified():
    """处理 docs/articles/ 下的 unified 文件（知识库GEO转化）"""
    log("检查 docs/articles unified 文件...")
    
    data = load_processed()
    docs_processed = set(data.get("docs_processed", []))
    
    if not DOCS_DIR.exists():
        log(f"警告: Docs目录不存在: {DOCS_DIR}")
        return []
...
"""
            ...
            output_path = ARTICLES_DIR / output_name
    ...
    if converted:
        log(f"共转化 {len(converted)} 篇 docs 文章")
    else:
        log("无新 docs 文章")
...
def git_push(converted_files):
    ...
    try:
        ...
        if result.returncode != 0:
            log(f"  Git add 警告: {result.stderr}")
        ...
        if result.returncode != 0:
            if "nothing to commit" in result.stdout:
                ...
                log(f"  Git commit 问题: {result.stderr}")
                return
...
def main():
    ...
    if not WORK_DIR.exists():
        log(f"错误: 仓库目录不存在: {WORK_DIR}")
        return
    
    if not ARTICLES_DIR.exists():
        log(f"错误: 文章目录不存在: {ARTICLES_DIR}")
        return
    
    # 处理公众号新文章
    converted_wechat = process_new_articles()
    
    # 处理 docs/articles unified 文件
    converted_docs = process_docs_unified()
    
    # 合并所有转化
    all_converted = converted_wechat + converted_docs
    
    # Git推送
    if all_converted:
        log(f"\n共转化 {len(all_converted)} 篇文章（公众号:{len(converted_wechat)} + docs:{len(converted_docs)}），开始Git推送...")
        git_push(all_converted)
    else:
        log("无新文章，跳过Git推送")
